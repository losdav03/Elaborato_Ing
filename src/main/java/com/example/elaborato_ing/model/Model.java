...
public class Model {
            ...
            for (Optionals item : auto.getOptionalSelezionabili()) {
                ...
                checkBox.selectedProperty().addListener((observable, oldValue, newValue) -> {
                    ...
                    if (newValue) {
                        ...
                        if (!costoScontato.getText().isEmpty()) {
                    ...
                    alert.showAndWait().ifPresent(response -> {
                        if (response == ButtonType.YES) {
                            try {
                                ...
                                openCloseFXML("/com/example/elaborato_ing/viewUtente/Login.fxml", event);
                            } catch (IOException e) {
                                throw new RuntimeException(e);
                            }
                        }
    ...
    private String creaStringaPreventivo(String[] parts) {
        if (Objects.equals(parts[16], String.valueOf(Stato.DA_PAGARE))) {
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd/MM/yyyy");
            LocalDate today = LocalDate.now();
            LocalDate dataInizioPreventivo = LocalDate.parse(parts[13], formatter);

            long daysBetween = ChronoUnit.DAYS.between(today, dataInizioPreventivo);

            if (daysBetween <= -20) {
                parts[16] = String.valueOf(Stato.SCADUTO);
            }
        }
            ...
            while ((riga = reader.readLine()) != null && !riga.isEmpty()) {
                ...
                if (campi[0].equals(idPreventivo)) {
                    if (campi.length >= 17 && campi[16].equals(Stato.DA_PAGARE.toString())) {
                        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd/MM/yyyy");
                        LocalDate today = LocalDate.now();
                        LocalDate dataInizioPreventivo = LocalDate.parse(campi[13], formatter);
                        ...
                        if (daysBetween <= -20) {
                            ...
                            campi[16] = String.valueOf(Stato.SCADUTO);
                        } else {
                        ...
                        switch (campi[16]) {
                            ...
                            case "SCADUTO":
                                ...
                                break;
                            case "RIFIUTATO":
                                alert.setContentText("Questo preventivo è stato rifiutato");
                                break;
                        }
                        alert.showAndWait();
                    }
